...
<div class="notes">
Tout d'abord, créer deux bases de données `publication` et `souscription` :
```
psql -c "CREATE DATABASE publication"
psql -c "CREATE DATABASE souscription"
```

Autorisez la réplication sur la localhost dans le fichier `pg_hba.conf`.

Dans la base de données `publication`, créer la publication et le slot de réplication :
```sql
CREATE PUBLICATION local_publication FOR TABLE t1;
CREATE TABLE t1 (id int GENERATED BY DEFAULT AS IDENTITY PRIMARY KEY, valeur int);
SELECT * FROM pg_create_logical_replication_slot('local_souscription','pgoutput');
```

Comme nous travaillons en local, il est nécessaire de créer le slot de réplication
manuellement. Il faudra créer la souscription de manière à ce qu'elle utilise le
slot de réplication que nous venons de créer. Si ce n'est pas fait, nous nous
exposons à un blocage de l'ordre de création de souscription. Ce problème
n'arrive pas lorsque l'on travaille sur deux instances séparées.

Ensuite, dans la base de données `souscription`, créer la table à répliquer et la souscription :
```sql
CREATE TABLE t1 (id int GENERATED BY DEFAULT AS IDENTITY PRIMARY KEY, valeur int);
CREATE SUBSCRIPTION souscription 
 CONNECTION 'host=127.0.0.1 port=5432 user=postgresql dbname=publication'
 PUBLICATION local_publication with (create_slot=false,slot_name='local_souscription');
```

Dans la base `publication`, insérez des données dans la table t1 et validez
que les données soient bien visibles dans la base `souscription`.
</div>

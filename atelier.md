...
<div class="notes">
```sql
publication=# CREATE PUBLICATION local_publication FOR TABLE t1;
publication=# CREATE TABLE t1 (id int GENERATED BY DEFAULT AS IDENTITY PRIMARY KEY, valeur int);
publication=# SELECT * FROM pg_create_logical_replication_slot('local_souscription','pgoutput');
```

```sql
local_souscription=# CREATE TABLE t1 (id int GENERATED BY DEFAULT AS IDENTITY PRIMARY KEY, valeur int);
local_souscription=# CREATE SUBSCRIPTION local_souscription CONNECTION 'host=127.0.0.1 port=5434 user=replilogique dbname=publication' PUBLICATION local_publication with (create_slot=false,slot_name='local_souscription');
```
